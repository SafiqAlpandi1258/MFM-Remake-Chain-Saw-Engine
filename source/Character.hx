	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'bf':
				...
				flipX = true;
			case 'sarvente':
				tex = Paths.getSparrowAtlas('sacredmass/sarvente_sheet','shared');
				frames = tex;
				...
				addOffset('idle');
				addOffset("singUP", 7, 68);
				addOffset("singRIGHT", 44, 17);
				addOffset("singLEFT", 27, 0);
				addOffset("singDOWN", 15, -52);
				addOffset('hey', -13, 6);

				playAnim('idle');

			case 'sarvente-dark':
				tex = Paths.getSparrowAtlas('sacredmass/sarvente_dark', 'shared');
				frames = tex;
			...
			case 'sarvente-lucifer':
				tex = Paths.getSparrowAtlas('sacredmass/smokinhotbabe', 'shared');
				frames = tex;
			...
			case 'ruv':
				tex = Paths.getSparrowAtlas('sacredmass/ruv_sheet','shared');
				frames = tex;
				...
				playAnim('idle');
			case 'selever':
				frames = Paths.getSparrowAtlas('sacredmass/fuckboi_sheet','shared');
				...
				animation.addByPrefix('hey', 'SelHey', 24);
